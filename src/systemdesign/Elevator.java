...
public class Elevator {
    ...
    public void call(int floor, Direction direction){
        ...
        Request newRequest = new Request(System.currentTimeMillis(), floor, direction);

        if(direction == Direction.UP){
            if(floor >= location){
                currentQueue.add(newRequest);
            }else{
                upQueue.add(newRequest);
            }
        }else {
    ...
    private void preProcessNextQueue() {
        if(getLowestTimeUpQueue() > getLowestDownTimeDownQueue()){
    ...
    private long getLowestTimeUpQueue() {
        ...
        for (Request r : upQueue){
            lowest = Math.min(lowest, r.time);
        }

        return lowest;
    }

    private long getLowestDownTimeDownQueue() {
        long lowest = Long.MAX_VALUE;
